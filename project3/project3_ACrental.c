...
// These arrays are taken to charge firstday cost , daily rate and put a weekly cost cap to get the total cost for that AC unit.
...
int main(void){
   ...
   // Take user input of AC type selected and make it start at range 0 for array index
    ...
    if (days == 0){ // no charge if no amount of days selected
        ...
        // Price for each full week 
        charge = fullWeeks * perWeekMax[ac_index];
        ...
        if (remainderDays > 0){
            ...
            if (fullWeeks == 0){
                // First day + remaining days before week cap
                remainderCost = firstDay[ac_index] +(remainderDays - 1) * dailyRate[ac_index];
            }
            else{
                // If days over 7 days charge per weekmax , use the daily rate for remanding days
                remainderCost = remainderDays * dailyRate[ac_index];
            }

            //If charge is more than per week max cost within the 7 days put the weeklycap
            if (remainderCost > perWeekMax[ac_index]){
                remainderCost = perWeekMax[ac_index];
            }

            charge += remainderCost;
        }
    }
