...
int search(int counselors[], int n){
    ...
    for(int i = 0; i < n - 1; i++){ // loops through the array for each assigned counselors (size n - 1)
        assigned_counselors += counselors[i]; // each counselor present is assigned to assigned_counselors
    }
